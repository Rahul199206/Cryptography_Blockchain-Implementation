...
public class database {
	...
	public static void sendData() {
        try{
            ...
            oos.writeObject(Main.blockchain);

            fos = new FileOutputStream("DOC_NAME.txt");
            oos = new ObjectOutputStream(fos);
            oos.writeObject(Main.doc_list);

            fos = new FileOutputStream("prev_hash.txt");
            oos = new ObjectOutputStream(fos);
            oos.writeObject(Main.previousHash);
            ...
            oos = new ObjectOutputStream(fos);
            oos.writeObject(Main.user_list);
    ...
    public static void receiveData(){
        try{
            ...
            if(new File("CHAIN.txt").length() != 0)
                Main.blockchain = (ArrayList<Block>)ois.readObject();

            fis = new FileInputStream("DOC_NAME.txt");
            ois = new ObjectInputStream(fis);

            if(new File("DOC_NAME.txt").length() != 0)
               Main.doc_list = (ArrayList<users>)ois.readObject();
            ...
            if(new File("Pat.txt").length() != 0)
                Main.user_list = (ArrayList<users>)ois.readObject();
            
            fis = new FileInputStream("prev_hash.txt");
            ois = new ObjectInputStream(fis);

            if(new File("prev_hash.txt").length() != 0)
                Main.previousHash = (String)ois.readObject();
